// === Aventurier de Mirval — game.js v10 enrichi ===
// Bloc 1/3 — Setup, état, utilitaires, stats & classes

console.log("game.js v10 enrichi — Bloc 1/3 chargé");
...
// RNG avec graine (xorshift simple)
const rng = (() => {
...
function setStats(){
  ...
  state.quests.side.forEach(q=>{
    const x=document.createElement('div'); x.className='stat'; x.innerHTML=`<b>${q.title}</b><span>${q.state}</span>`; ui.quests.appendChild(x);
  });
}
...
function chooseClass(){
  ...
  addChoice('🛡️ Guerrier', ()=> pick('Guerrier','Puissance',3,{ name:'Frappe vaillante', cooldown:3, cd:0, desc:'Attaque puissante', use:(e)=>{ const dmg=rng.between(2,6)+rng.between(2,6)+state.level; e.hp-=dmg; write(`💥 Frappe vaillante : -${dmg} PV`,'good'); } }), true);
  ...
  addChoice('⚕️ Paladin', ()=> pick('Paladin','Esprit',2,{ name:'Lumière', cooldown:3, cd:0, desc:'Soigne', use:()=>{ heal(rng.between(3,8)+state.level); } }));
...
// === Bloc 2/3 — Combat, bestiaire, temps, exploration générique & PNJ de base ===

// --- Modificateurs de stats (cohérent avec Puissance/Agilite/Esprit) ---
...
function combatTurn(){
  ...
  // Compétence de classe (issue du choix de classe du Bloc 1/3)
...
// --- Sélection pondérée avec mémo pour limiter la redondance ---
...
// === Bloc 3/3 — Réputation, quêtes, fins, setup et démarrage ===

// --- Réputation (influence PNJ & factions) ---
function rep(n){ 
  state.rep += n; 
  setStats(); 
  if(n>0) write(`Réputation +${n}`,"good"); 
  if(n<0) write(`Réputation ${n}`,"bad"); 
}
...
// --- QoL : gestion du cooldown de compétence ---
...
// --- PWA service worker (si activé côté projet) ---
