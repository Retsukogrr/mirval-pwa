/* =========================================================
   Aventurier de Mirval — v10 (classic, robuste, sans sauvegarde)
   game.js — Bloc 1/4
   ========================================================= */

// Marqueur (utile si tu utilises un play.html de diagnostic)
window.__MIRVAL_JS_OK = true;

// ----------------- Réveil écran (non bloquant) -----------------
let wakeLock;
...
function initialState(){
  return {
    name:"Eldarion",
    cls:"—",
    // Stats renommées lisibles
    attrs:{STR:1, AGI:1, WIS:1}, // FOR/DEX/ESPRIT à l’écran
...
// ----------------- Choix de classe (forcé & auto-réessai) -----------------
function chooseClass(){
  ...
  // failsafe : si aucun bouton rendu (rare), retente au frame suivant
...
function setup(isNew=false){
  ...
  const needsClass = !state.cls || state.cls==='—' || !classes.includes(state.cls);
...
(function boot(){
  try{
    ...
    requestAnimationFrame(()=>{
      try{ bindUI(); if (!state) state = initialState(); setup(true); }catch(e2){ console.error('Boot retry failed:', e2); }
    });
  }
})();
/* =========================================================
   game.js — Bloc 2/4 : Combat, statuts, bestiaire, boss
   ========================================================= */
...
function combatTurn(){
  ...
  addChoice(`🧪 Potion (${state.potions})`, ()=>{
    ...
    state.potions--; heal(rng.between(8,12)); if(e.hp>0) enemyAttack(); combatTurn();
  });
  addChoice('🏃 Fuir', ()=>{
...
/* =========================================================
   game.js — Bloc 3/4 : Actions, PNJ, événements, exploration
   ========================================================= */
...
function eventSmith(){
  ...
  // Vente rapide (3 premiers objets, estimation simple)
  state.inventory.slice(0,3).forEach(it=>{
    const price = Math.max(1, (/armure/i.test(it.desc||'')?2:0) + (/attaque/i.test(it.desc||'')?2:0) ) || 1;
...
/* =========================================================
   game.js — Bloc 4/4 : Fins, game over, cooldown, fin
   ========================================================= */
