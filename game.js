// === Aventurier de Mirval — game.js v10 (classique, complet, sans sauvegarde) ===
console.log("Mirval v10 (classic)");
...
function eventSmith(){
  ...
  // Vente rapide (3 premiers objets)
...
function setup(isNew=false){
  ...
  if(isNew || ui.log.childElementCount===0 || needsClass){
    ...
    chooseClass();
    // failsafe : si pas de boutons, réaffiche après le frame
...
// Cooldown -1 à chaque exploration (classique)
...
// Boot DOM-ready (classique)
(function boot(){
  const start=()=>{ try{ bindUI(); if(!state.cls) state.cls='—'; setup(true); }catch(e){ console.error(e); setTimeout(()=>{ try{ bindUI(); setup(true); }catch(_){} },0); } };
